...
public class FamilyTree
{
    ...
    private static class TreeNode
    {
        ...
        TreeNode getNodeWithName(String targetName)
        {
            ...
            for (TreeNode trave: children) {
            	TreeNode result = trave.getNodeWithName(targetName);
            	if (result != null) {
            		return result;
            	}
            }    
        ...
        private void ancestorsFind(TreeNode node, ArrayList<TreeNode> ancestors) {
        	if (node.parent == null) {
        		return;
        	}
        	ancestors.add(node.parent);
        	ancestorsFind(node.parent, ancestors);
	...
	private void addLine(String line) throws TreeException {
		...
		if (colonIndex < 0) {
//			System.out.print(colonIndex);
			 throw new TreeException("Issue with the index of colon - less than zero");
		}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("nothing in here"); //with a useful message
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("nothing in here");
	...
	public static void main(String[] args)
	{
		try
		{
			...
			TreeNode ancestor = tree.getMostRecentCommonAncestor("Dora", "Drogo");
			System.out.println("Most recent common ancestor of Dora and Drogo is " + ancestor.getName());
		}
		catch (IOException x)
		{
